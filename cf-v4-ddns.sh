...
if [ "$CNAME" = "" ]; then
  ...
  WAN_IP=`curl --http2 -H 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=${CNAME}&type=A" | jq -r '.Answer[] | select(.type==1) | .data'`
fi 
